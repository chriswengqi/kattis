...
void solve()
{
    ...
    // vertex i = time * n + i
    max_flow mf(n * (time + 1) + 3);
    // put a dummy
    mf.add_edge(0, n * (time + 1) + 2, ppl);
    for (int i = 0; i <= time; i++)
    {
        mf.add_edge(n * (time + 1) + 2, i * n + start, ppl);
    }
    for (int i = 1; i <= n; i++)
    {
        for (auto [v, p, ttt] : roads[i])

        {
            for (int tt = 0; tt <= time; tt++)
            {
                if (tt + ttt > time)
                    break;
                mf.add_edge(n * tt + i, n * (tt + ttt) + v, p);
            }
        }
    }
    // cout << "Here" << endl;
    ...
    for (int a : dsts)
    {
        for (int tt = 0; tt <= time; tt++)
        {
            mf.add_edge(n * tt + a, n * (time + 1) + 1, INF);
        }
    }

    int ans = mf.dinic(0, n * (time + 1) + 1);
