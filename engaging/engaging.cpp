...
signed main()
{
    ...
    costMatrix.resize(max(n, m), vector<int>(max(m, n), 0));
    // customers: job,n presents: workers, m
    // assert(n <= m), if m > n just swap
    for (int i = 0; i < r; i++)
    {
        int a, b, c;
        cin >> a >> b >> c;
    ...
    // ans[w] = customer assigned to w-th present
    // ans[1] = 0
    // ans[2] = 1
    vi ans = hungarian(costMatrix);
    ...
    for (int i = 0; i < m; i++)
    {
        if (ans[i] != -1 && costMatrix[ans[i]][i] != 0)
        {
            total++;
            cost -= costMatrix[ans[i]][i];
            assignments.pb({ans[i] + 1, i + 1});
        }
    }

    cout << cost << endl;
    cout << total << endl;
    for (auto [a, b] : assignments)
    {
        cout << a << " " << b << endl;
    }
