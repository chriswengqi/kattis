...
signed main()
{
    ...
    max_flow mf(n + m + p + 2);
    // children 1 - n, toys n + 1 - n + m, categories n + m + 1 - n + m + p,
    ...
    for (int i = 1; i <= p; i++)
    {
        ...
        for (int j = 0; j < s; j++)
        {
            int t;
            cin >> t;
            mf.add_edge(n + t, n + m + i, 1);
            dic.erase(t);
        }
        int t;
        cin >> t;
        mf.add_edge(n + m + i, n + m + p + 1, t);
    }

    for (int a : dic)
    {
        mf.add_edge(n + a, n + m + p + 1, 1);
    }
    ...
    //     for (int j : mf.AL[i])
    //     {
    //         auto [v, cap, __] = mf.EL[j];
    //         cout << v << "," << cap << " ";
    //     }
    //     cout << endl;
    // }

    cout << mf.dinic(0, n + m + p + 1) << endl;
    return 0;
}