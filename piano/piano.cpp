...
void solve()
{
    ...
    max_flow mf1(n + 102);
    max_flow mf2(n + 102);
    // 1 - 100 days, 101 -> 100 + n -> pianos, 101 + n -> sink
    for (int i = 1; i <= 100; i++)
    {
        mf1.add_edge(0, i, workers);
        mf2.add_edge(0, i, workers);
    }
    ...
    for (int i = 1; i <= n; i++)
    {
        ...
        {
            if (j % 7 != 0 && j % 7 != 6)
                mf1.add_edge(j, 100 + i, 1);
            mf2.add_edge(j, 100 + i, 1);
        }
        mf1.add_edge(100 + i, 101 + n, 1);
        mf2.add_edge(100 + i, 101 + n, 1);
    }

    int ans = mf1.dinic(0, 101 + n);
    if (ans == n)
    {
        cout << "fine" << endl;
        return;
