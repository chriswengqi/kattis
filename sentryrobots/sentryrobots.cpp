...
void solve()
{
    ...
    vector<vvi> grid2(n, vvi(m, vi(2, -1)));
    int curr_x = 0;
    for (int i = 0; i < n; i++)
    {
        for (int j = 0; j < m; j++)
    ...
    max_flow mf(curr_x + curr_y + 2);
    int src = 0;
    int dst = curr_x + curr_y + 1;
    ...
    for (int i = 0; i < n; i++)
    {
        for (int j = 0; j < m; j++)
        {
            if (grid2[i][j] != vi{-1, -1})
            {
                mf.add_edge(grid2[i][j][0] + 1, curr_x + grid2[i][j][1] + 1, 1);
            }
        }
    }
    ...
    for (int i = 1; i <= curr_y; i++)
        mf.add_edge(curr_x + i, dst, 1);

    cout << mf.dinic(src, dst) << endl;
}
