...
signed main()
{
    ...
    while (n != -1)
    {
        ...
        if (arr[n - 1] + matches_left * 2 <= maxx)
        {
            cout << "NO" << endl;
            cin >> n;
            continue;
        ...
        for (int i = 1; i <= m; i++)
        {
            if (matches[i - 1].first != n && matches[i - 1].second != n)
            {
                mf.add_edge(0, i, 2);
            }
        }
        ...
        for (int i = 1; i <= m; i++)
        {
            if (matches[i - 1].first != n && matches[i - 1].second != n)
            {
                mf.add_edge(i, m + matches[i - 1].first, 2);
                mf.add_edge(i, m + matches[i - 1].second, 2);
            }
        }
        ...
        for (int i = 1; i <= n; i++)
        {
            if (i != n)
            {
                mf.add_edge(m + i, n + m + 1, arr[n - 1] + 2 * matches_left - arr[i - 1] - 1);
            }
        }

        int ans = mf.dinic(0, n + m + 1);
        if (ans < (m - matches_left) * 2)
        {
            cout << "NO" << endl;
        }
        else
        {
            for (int i = 1; i <= m; i++)
            {
                if (matches[i - 1].first == n)
                {
                    cout << 0 << " ";
                }
                else if (matches[i - 1].second == n)
                {
                    cout << 2 << " ";
                }
                else
                {
                    int v = mf.edge_map[i * (m + n + 2) + m + matches[i - 1].first];
                    cout << 2 - get<2>(mf.EL[v]) << " ";
                }
            }
            cout << endl;
        }
