    ...
    for (int i = 1; i <= t; i++)
    {
        ...
        for (int j = 0; j < ss; j++)
        {
            ...
            if (dic.count(tt))
            {
                if (dic[tt] <= r)
                {
                    mf.add_edge(dic[tt], r + f + i, 1, false);
                }
                else if (dic[tt] > r + f)
                {
                    mf.add_edge(r + f + i, dic[tt], INF, false);
                }
                else
                {
                    mf.add_edge(r + f + i, dic[tt], 1, false);
                }
            }
            else
            {
                mf.add_edge(ctr, r + f + i, INF, false);
                dic[tt] = ctr++;
            }
        }
    }

    int ans = mf.dinic(0, s + t + 1);
    cout << ans << endl;
