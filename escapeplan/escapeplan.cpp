    ...
    while (n != 0)
    {
        ...
        for (int t = 0; t < 3; t++)
        {
            ...
            for (int i = 0; i < n; i++)
            {
                for (int j = 0; j < m; j++)
                {
                    double dsq = (robots[i].first - holes[j].first) * (robots[i].first - holes[j].first) + (robots[i].second - holes[j].second) * (robots[i].second - holes[j].second);
                    if (sqrt(dsq) <= ts[t] * 10)
                    {
                        AL[i].insert(j + n);
                    }
            ...
            cout << "In " << ts[t] << " seconds " << MCBM << " robot(s) can escape" << endl;
        }
